...
export const mapIssueToUiModel = (issue = {}) => {
  const evidenceMedia = Array.isArray(issue.evidenceMedia)
    ? issue.evidenceMedia
    : [];
  const evidenceImages = evidenceMedia
    .filter(
      (media) =>
        String(media.mediaType || "IMAGE").toUpperCase() === "IMAGE" &&
        media.mediaUrl,
    )
  ...
  return {
    ...
    coordinates:
      issue.latitude != null && issue.longitude != null
        ? { lat: Number(issue.latitude), lng: Number(issue.longitude) }
...
export const listIssues = async () =>
  (await apiClient.get("/api/issues")).map(mapIssueToUiModel);
export const getIssue = async (id) =>
  mapIssueToUiModel(await apiClient.get(`/api/issues/${id}`));
export const getCitizenIssues = async (citizenId) =>
...
export const getIssuesByStatus = async (status) =>
  (
    await apiClient.get(
      `/api/issues/status/${statusToBackend[status] || status}`,
    )
  ).map(mapIssueToUiModel);
export const getIssuesByPriority = async (priority) =>
  (
    await apiClient.get(`/api/issues/priority/${priorityToBackend(priority)}`)
  ).map(mapIssueToUiModel);
export const createIssue = async (issue) =>
  mapIssueToUiModel(
    await apiClient.post("/api/issues", mapIssueToApiModel(issue)),
  );
...
export const updateIssuePriority = async (id, priority) =>
  mapIssueToUiModel(
    await apiClient.patch(
      `/api/issues/${id}/priority?priority=${encodeURIComponent(priorityToBackend(priority))}`,
    ),
  );
export const uploadEvidence = (id, formData) =>
  apiClient.post(`/api/issues/${id}/evidence`, formData);

...
export const DataProvider = ({ children }) => {
  ...
  const addIssue = async newIssue => {
    if (LIVE_API && isAuthenticated) { const created = await issueApi.createIssue(newIssue); const uiIssue = { ...created, category: newIssue.category, categoryLabel: newIssue.categoryLabel, district: newIssue.district, submittedBy: `${currentUser.name} (Citizen)`, images: newIssue.images || [] }; setIssues(prev => [uiIssue, ...prev]); return uiIssue; }
  ...
  const verifyIssue = async (id, { status, priority, nodalRemarks, assignedUniversity }) => { if (LIVE_API && isAuthenticated) { let updated; if (status) updated = await issueApi.updateIssueStatus(id, status); if (priority) updated = await issueApi.updateIssuePriority(id, priority); if (updated) setIssues(prev => prev.map(i => i.id === id ? { ...i, ...updated, nodalRemarks: nodalRemarks || i.nodalRemarks, assignedUniversity: assignedUniversity || i.assignedUniversity } : i)); return updated; } setIssues(prev => prev.map(i => i.id === id ? { ...i, status, priority: priority || i.priority, nodalRemarks: nodalRemarks || i.nodalRemarks, assignedUniversity: assignedUniversity || i.assignedUniversity, timeline: [...(i.timeline || []), { status, date: new Date().toISOString().split('T')[0], remark: nodalRemarks || `Status updated to ${status} by Nodal Officer` }] } : i)); };

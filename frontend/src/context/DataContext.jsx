...
export const DataProvider = ({ children }) => {
  ...
  useEffect(() => {
    const storageKey = currentUser?.id
      ? `samadhansetu_upvotes_${currentUser.id}`
      : null;
    if (!storageKey) {
      setLikedIssueIds(new Set());
      return;
    }
    try {
  ...
  useEffect(() => {
    ...
    dashboardApi
      .getSummary()
      .then((summary) => {
        if (!cancelled) setStats({ ...EMPTY_STATS, ...(summary || {}) });
      })
      .catch(() => {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        if (!cancelled)
          setDataError(error.message || "Unable to load live platform data.");
      } finally {
  ...
  const addIssue = async (newIssue) => {
    if (!isAuthenticated)
      throw new Error(
        "Live API authentication is required to create an issue.",
      );
    const created = await issueApi.createIssue(newIssue);
    const uiIssue = {
      ...created,
      category: newIssue.category,
      categoryLabel: newIssue.categoryLabel,
      district: newIssue.district,
      submittedBy: `${currentUser.name} (Citizen)`,
      images: created.images?.length ? created.images : newIssue.images || [],
    };
  ...
  const upvoteIssue = (id) => {
    ...
    setIssues((prev) =>
      prev.map((i) =>
        i.id === id ? { ...i, upvotes: Number(i.upvotes || 0) + 1 } : i,
      ),
    );
    return true;
  };
  ...
  const verifyIssue = async (
    id,
    { status, priority, nodalRemarks, assignedUniversity },
  ) => {
    if (!LIVE_API || !isAuthenticated)
        ...
        prev.map((i) =>
          i.id === id
            ? {
                ...i,
                ...updated,
                nodalRemarks: nodalRemarks || i.nodalRemarks,
  ...
  const updateTaskStatus = async (projectId, taskId, newStatus) => {
    ...
    if (!task) return null;
    const updated = await projectApi.updateTask(taskId, {
      title: task.title,
    ...
    setProjects((prev) =>
      prev.map((p) =>
        p.id === projectId
          ? {
              ...p,
              kanbanTasks: p.kanbanTasks.map((t) =>
  ...
  const addKanbanTask = async (projectId, taskData) => {
    ...
    setProjects((prev) =>
      prev.map((p) =>
        p.id === projectId
          ? { ...p, kanbanTasks: [...(p.kanbanTasks || []), created] }
          : p,
      ),
    );
    return created;
  };
  const sponsorProject = async (
    projectId,
    amount,
    sponsorName,
    organizationId,
    ...
    const created = await industryApi.createSponsorship({
      organizationId,
      projectId,
  ...
  const updateMilestone = async (projectId, index, newStatus) => {
    const milestone = projects.find((p) => p.id === projectId)?.milestones?.[
      index
    ];
    if (!milestone?.id) return null;
    const updated = await projectApi.updateMilestone(milestone.id, {

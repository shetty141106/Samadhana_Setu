...
const normalizeRole = role => { const value = String(role || '').toLowerCase(); return Object.values(ROLES).includes(value) ? value : ROLES.CITIZEN; };
...
export const AuthProvider = ({ children }) => {
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    notificationApi.listUserNotifications(currentUser.id).then(items => { if (!cancelled && Array.isArray(items)) setNotifications(items.map(notificationToUi)); }).catch(() => {});
  ...
  const login = async credentials => {
    ...
    try {
      if (!LIVE_AUTH) { const mock = MOCK_USERS.find(u => u.email.toLowerCase() === credentials.email.toLowerCase()) || MOCK_USERS[0]; setDemoRole(mock.role); return { ...mock, token: 'demo-token' }; }
      const response = await authApi.login(credentials); const nextSession = { ...response, role: normalizeRole(response.role) };
      localStorage.setItem(SESSION_KEY, JSON.stringify(nextSession)); setSession(nextSession); return nextSession;
  ...
  const logout = () => { clearAuthToken(); localStorage.removeItem(SESSION_KEY); setSession(null); setDemoRole(ROLES.CITIZEN); setNotifications(MOCK_NOTIFICATIONS); };
  const switchRole = newRole => { if (Object.values(ROLES).includes(newRole)) setDemoRole(newRole); };

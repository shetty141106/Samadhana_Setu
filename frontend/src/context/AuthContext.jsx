...
const normalizeRole = (role) => {
  ...
  return Object.values(ROLES).includes(value) ? value : ROLES.CITIZEN;
};
...
export const AuthProvider = ({ children }) => {
  ...
  const switchRole = (newRole) => {
    if (Object.values(ROLES).includes(newRole)) setDemoRole(newRole);
  };
  ...
  const value = useMemo(() => ({
    currentRole, currentUser, roleConfig, switchRole, login, register, logout,
    isAuthenticated, authLoading, authError, language, toggleLanguage,
    notifications, markNotificationAsRead, markAllNotificationsAsRead,
    unreadCount: notifications.filter(n => !n.read).length
  }), [currentRole, currentUser, roleConfig, session, isAuthenticated, authLoading, authError, language, notifications]);

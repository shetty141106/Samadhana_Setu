...
const notificationToUi = (n) => ({
  ...n,
  read: Boolean(n.read ?? n.readStatus),
  readStatus: Boolean(n.read ?? n.readStatus),
});
...
export const AuthProvider = ({ children }) => {
  ...
  const currentUser = session
    ? { ...session, role: currentRole, id: session.userId ?? session.id }
    : null;
  ...
  useEffect(() => {
    if (!LIVE_AUTH || !isAuthenticated || !currentUser?.id) {
      setNotifications([]);
      return;
    }
    let cancelled = false;
    notificationApi
      .listUserNotifications(currentUser.id)
      .then((items) => {
        if (!cancelled && Array.isArray(items))
          setNotifications(items.map(notificationToUi));
      })
      .catch((error) => {
        if (error?.status === 401)
  ...
  const login = async (credentials) => {
    ...
    try {
      if (!LIVE_AUTH)
        throw new Error(
          "Live authentication is disabled. Set VITE_ENABLE_LIVE_API=true.",
        );
  ...
  const markNotificationAsRead = async (id) => {
    if (LIVE_AUTH && isAuthenticated)
      await notificationApi.markAsRead(id).catch((error) => {
        if (error?.status === 401)
          window.dispatchEvent(new Event("samadhansetu-auth-invalidated"));
      });
    setNotifications((prev) =>
      prev.map((n) =>
        n.id === id ? { ...n, read: true, readStatus: true } : n,
      ),
    );
  };
  const markAllNotificationsAsRead = async () => {
    const unread = notifications.filter((n) => !n.read && !n.readStatus);
    if (LIVE_AUTH && isAuthenticated)
      await Promise.allSettled(
        unread.map((n) => notificationApi.markAsRead(n.id)),
      );
    setNotifications((prev) =>
      prev.map((n) => ({ ...n, read: true, readStatus: true })),
    );
  };
  ...
  const value = useMemo(
    () => ({
      ...
      unreadCount: notifications.filter((n) => !n.read && !n.readStatus).length,
    }),
    [
      currentRole,
      currentUser,

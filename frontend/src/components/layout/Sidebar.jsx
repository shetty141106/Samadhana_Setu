...
export const Sidebar = ({
    ...
    switch (currentRole) {
      ...
      case ROLES.ADMIN:
        return [
          ...
          { id: "admin-issues", label: "All Issues", icon: ClipboardList },
  ...
  return (
    <>
      ...
      <aside
        className={`fixed md:sticky top-16 md:top-18 left-0 z-30 h-[calc(100vh-4rem)] md:h-[calc(100vh-4.5rem)] w-64 bg-jh-earth-50 border-r border-jh-earth-200 flex flex-col justify-between transition-transform duration-300 md:translate-x-0 ${sidebarOpen ? "translate-x-0" : "-translate-x-full"}`}
      >
        <div className="p-4 border-b border-jh-earth-200 bg-white/70">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-xl border-2 border-jh-green-800/30 bg-jh-green-100 text-jh-green-900 grid place-items-center">
              <User className="w-5 h-5" />
            </div>
            <div className="min-w-0 flex-1">
              <p className="text-xs font-bold text-jh-green-950 truncate">
                {currentUser?.name || "User"}
              </p>
              <p className="text-[11px] text-jh-earth-600 truncate">
                {roleConfig?.portalTitle || "Portal"}
              </p>
              {roleConfig && (
                <span
                  className={`inline-block mt-1 text-[9.5px] font-bold px-2 rounded-full border ${roleConfig.badgeColor}`}
                >
                  {roleConfig.label}
                </span>
              )}
          ...
          {navItems.map((item) => {
            ...
            return (
              <button
                ...
                className={`w-full flex items-center justify-between px-3.5 py-2.5 rounded-xl text-xs font-semibold transition-all ${isActive ? "bg-jh-green-900 text-white shadow-xs" : item.highlight ? "bg-jh-terracotta-100/70 text-jh-terracotta-800 hover:bg-jh-terracotta-200/80 border border-jh-terracotta-300/60" : "text-jh-charcoal hover:bg-jh-earth-200/80"}`}
              >
                <div className="flex items-center gap-3">
                  <Icon
                    className={`w-4 h-4 ${isActive ? "text-white" : item.highlight ? "text-jh-terracotta-600" : "text-jh-green-800"}`}
                  />

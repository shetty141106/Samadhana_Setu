...
export const IssueForm = ({ onSuccess, onCancel }) => {
  ...
  const handleAddSamplePhoto = (url) => {
    if (!previewImages.includes(url)) setPreviewImages([...previewImages, url]);
  };
  ...
  const handleDistrictChange = (district) => {
    ...
    // The picker will recenter on the newly selected district.
  ...
  return (
    <form onSubmit={handleSubmit} className="bg-white rounded-2xl border border-jh-earth-200 shadow-jh-soft p-6 md:p-8 space-y-6">
      ...
      <div>
        <label className="block text-xs font-bold uppercase tracking-wider text-jh-earth-800 mb-1.5">Issue Title *</label>
        <input
          type="text"
          required
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div>
          <label className="block text-xs font-bold uppercase tracking-wider text-jh-earth-800 mb-1.5">Category / Domain *</label>
          <select
            value={formData.category}
            onChange={(e) => setFormData({ ...formData, category: e.target.value })}
            className="w-full px-4 py-2.5 text-sm bg-jh-earth-50 border border-jh-earth-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-jh-green-700 focus:bg-white text-jh-charcoal"
          >
            {ISSUE_CATEGORIES.map(cat => <option key={cat.id} value={cat.id}>{cat.label}</option>)}
          </select>
        </div>

        <div>
          <label className="block text-xs font-bold uppercase tracking-wider text-jh-earth-800 mb-1.5">Jharkhand District *</label>
          <select
            value={formData.district}
            onChange={(e) => handleDistrictChange(e.target.value)}
            className="w-full px-4 py-2.5 text-sm bg-jh-earth-50 border border-jh-earth-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-jh-green-700 focus:bg-white text-jh-charcoal"
          >
            {JHARKHAND_DISTRICTS.map(dist => <option key={dist} value={dist}>{dist}</option>)}
          </select>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="md:col-span-2">
          <label className="block text-xs font-bold uppercase tracking-wider text-jh-earth-800 mb-1.5">Specific Landmark / Block / Village *</label>
        ...
        <div>
          <label className="block text-xs font-bold uppercase tracking-wider text-jh-earth-800 mb-1.5">Estimated Urgency</label>
          <select
      ...
      <div>
        <label className="block text-xs font-bold uppercase tracking-wider text-jh-earth-800 mb-1.5">Exact Issue Location *</label>
        <LocationPicker
          district={formData.district}
        ...
        />
        {!selectedCoordinates && (
          <p className="mt-1.5 text-[11px] font-medium text-jh-terracotta-700">Select the exact location on the map before submitting.</p>
        )}
      </div>

      <div>
        <label className="block text-xs font-bold uppercase tracking-wider text-jh-earth-800 mb-1.5">Detailed Description & Impact on Ground *</label>
        <textarea
          rows={4}
          required
      ...
      <div>
        <label className="block text-xs font-bold uppercase tracking-wider text-jh-earth-800 mb-1.5">Photographic Evidence (Geo-Tagged / On-Site)</label>
        <div className="mb-3">
          ...
          <div className="flex flex-wrap gap-2">
            {sampleForestPhotos.map((photo, i) => (
              <button key={i} type="button" onClick={() => handleAddSamplePhoto(photo.url)} className="text-[11px] px-2.5 py-1 rounded-lg border border-jh-green-700/40 bg-jh-green-50 text-jh-green-900 hover:bg-jh-green-100 font-medium transition-colors">
                + {photo.label}
              </button>
            ))}
          </div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-3">
            {previewImages.map((imgUrl, index) => (
              <div key={index} className="relative group rounded-xl overflow-hidden border border-jh-earth-300 h-24">
                ...
                <button type="button" onClick={() => handleRemovePhoto(index)} className="absolute top-1 right-1 p-1 bg-black/70 text-white rounded-md hover:bg-red-600 transition-colors">
      ...
      <div className="pt-4 border-t border-jh-earth-200 flex items-center justify-end gap-3">
        ...
        <Button type="submit" variant="primary" size="lg" disabled={isSubmitting || !selectedCoordinates} icon={isSubmitting ? null : Camera}>

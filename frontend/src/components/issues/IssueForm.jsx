...
export const IssueForm = ({ onSuccess, onCancel }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (
      ...
      setSubmitError(
        "Please complete the title, description, district, landmark and exact map location before submitting.",
      );
      return;
    }
    if (!currentUser?.email) {
      setSubmitError(
        "Citizen session is missing. Please sign in again and retry.",
      );
      return;
    }
    ...
    try {
      ...
      setSubmitError(
        detail || "Unable to submit the grievance. Please try again.",
      );
    } finally {
  ...
  if (showSuccessModal)
    return (
      <div className="bg-white rounded-2xl border border-jh-green-200 p-8 text-center max-w-lg mx-auto shadow-jh-card animate-in zoom-in-95">
        ...
        <h3 className="text-xl font-bold text-jh-green-950 mb-2">
          Grievance Registered Successfully!
        </h3>
        <p className="text-xs text-jh-earth-700 leading-relaxed mb-4">
          Your issue has been routed to the District Nodal Verification Desk for
          ground inspection and academic solution mapping.
        </p>
        {aiResult && (
          <div className="text-left rounded-xl border border-jh-green-200 bg-jh-green-50 p-3 mb-5 text-xs">
            <div className="flex items-center gap-2 font-bold text-jh-green-900 mb-1">
              <Sparkles className="w-4 h-4" />
              AI triage completed
            </div>
            <p>
              <strong>Category:</strong> {aiResult.categoryTag || "—"} &nbsp;{" "}
            ...
            {aiResult.duplicateMatch?.found && (
              <p className="text-amber-800 mt-1">
                Possible duplicate detected (
                {aiResult.duplicateMatch.similarityPercentage}%).
              </p>
            )}
          </div>
        )}
  ...
  return (
    <form
      ...
      className="bg-white rounded-2xl border border-jh-earth-200 shadow-jh-soft p-6 md:p-8 space-y-6"
    >
      <div className="border-b border-jh-earth-200 pb-4">
        ...
        <p className="text-xs text-jh-earth-600 mt-1">
          Provide accurate details and photos to help our district nodal
          officers and university research teams deploy fast interventions.
        </p>
      </div>
      {submitError && (
        <div
        ...
        <div className="md:col-span-2">
          ...
          <div className="relative">
            ...
            <input
              type="text"
              required
              placeholder="Enter landmark, block or village"
              value={formData.locationName}
              onChange={(e) =>
                setFormData({ ...formData, locationName: e.target.value })
              }
              className="w-full pl-10 pr-4 py-2.5 text-sm bg-jh-earth-50 border border-jh-earth-300 rounded-xl"
            />
          </div>
        </div>
        <div>
      ...
      <div>
        ...
        />
        {!selectedCoordinates && (
          <p className="mt-1.5 text-[11px] font-medium text-jh-terracotta-700">
            Select the exact location on the map before submitting.
          </p>
        )}
      </div>
      <div>
        ...
        <textarea
          rows={4}
          required
          placeholder="Describe the issue and its impact on the community or environment"
      ...
      <div>
        ...
        <p className="text-[11px] text-jh-earth-600 mb-2">
          Upload up to 4 field photographs. Evidence is optional.
        </p>
        <input
          ref={fileInputRef}
          type="file"
        ...
        <button
          ...
          className="w-full sm:w-auto inline-flex items-center justify-center gap-2 rounded-xl border border-dashed border-jh-green-700 bg-jh-green-50 px-4 py-3 text-xs font-bold text-jh-green-900 hover:bg-jh-green-100 disabled:opacity-40 transition-colors"
        >
          <Upload className="w-4 h-4" /> Choose Images from Device
        </button>
        {mediaItems.length > 0 && (
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mt-3">
            {mediaItems.map((item, index) => (
              <div
                ...
                className="relative rounded-xl overflow-hidden border border-jh-earth-300 h-24"
              >
                <img
                  src={item.previewUrl}
                ...
                <button
                  ...
                  className="absolute top-1 right-1 p-1 bg-black/70 text-white rounded-md"
                >
                  <X className="w-3.5 h-3.5" />
                </button>
                {item.file && (
                  <span className="absolute bottom-1 left-1 inline-flex items-center gap-1 rounded-md bg-black/70 px-1.5 py-0.5 text-[9px] text-white">
                    <ImageIcon className="w-3 h-3" /> Device
                  </span>
                )}
              </div>
            ))}
      ...
      <div className="pt-4 border-t border-jh-earth-200 flex items-center justify-end gap-3">
        ...
        >
          {isSubmitting
            ...
            : "Submit Issue for Nodal Verification"}
        </Button>
      </div>
    </form>
  );

...
import { MapPin, ThumbsUp, ArrowRight, Building, School, BrainCircuit } from 'lucide-react';
...
export const IssueCard = ({ issue, onSelect, onUpvote }) => {
  ...
  return (
    <div className="bg-white rounded-2xl border border-jh-earth-200/90 shadow-jh-soft overflow-hidden hover:shadow-jh-card transition-all duration-200 flex flex-col justify-between group">
      <div>
        <div className="relative h-44 w-full overflow-hidden bg-jh-earth-200">
          ...
          <div className="absolute bottom-3 left-3 right-3 flex items-center justify-between text-white text-xs">
            <div className="flex items-center gap-1.5 font-medium drop-shadow-sm"><MapPin className="w-3.5 h-3.5 text-jh-terracotta-400" /><span className="truncate">{issue.district || issue.location || 'Location unavailable'}</span></div>
            {issue.priority && <span className={`px-2 py-0.5 rounded-full text-[10px] font-bold ${String(issue.priority).toUpperCase() === 'CRITICAL' ? 'bg-red-600 text-white' : 'bg-amber-600 text-white'}`}>{issue.priority} Priority</span>}
          </div>
        </div>
        <div className="p-5">
      ...
      {showAI && <div className="px-4 pb-4"><AIIntelligencePanel issue={issue} /></div>}
      <div className="px-5 py-3.5 bg-jh-earth-50/70 border-t border-jh-earth-100 flex items-center justify-between gap-3">
        <button onClick={(e) => { e.stopPropagation(); if (onUpvote) onUpvote(issue.id); }} className="flex items-center gap-1.5 text-xs font-semibold text-jh-earth-700 hover:text-jh-green-900 transition-colors" title="Support this civic issue"><ThumbsUp className="w-3.5 h-3.5 text-jh-green-700" /><span>{issue.upvotes ?? 0}</span></button>

      ...
      <div>
        <div className="relative h-44 w-full overflow-hidden bg-jh-earth-200">
          ...
          <div className="absolute bottom-3 left-3 right-3 flex items-center justify-between text-white text-xs gap-2">
            <div className="flex items-center gap-1.5 font-medium drop-shadow-sm min-w-0">
              <MapPin className="w-3.5 h-3.5 text-jh-terracotta-400 shrink-0" />
              <span className="truncate">
                {issue.district || issue.location || "Location unavailable"}
              </span>
            </div>
            {issue.priority && (
              <span
                className={`px-2 py-0.5 rounded-full text-[10px] font-bold shrink-0 ${String(issue.priority).toUpperCase() === "CRITICAL" ? "bg-red-600 text-white" : "bg-amber-600 text-white"}`}
              >
        ...
        <div className="p-5">
          <div className="text-[11px] font-bold uppercase tracking-wider text-jh-terracotta-700 mb-1">
            {issue.categoryLabel || issue.category || "Civic Issue"}
          </div>

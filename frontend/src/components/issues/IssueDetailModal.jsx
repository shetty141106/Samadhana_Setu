...
export const IssueDetailModal = ({ issue, isOpen, onClose, onNavigate }) => {
  ...
  useEffect(() => {
    ...
    industryApi
      ...
      .then((items) => {
        if (!cancelled)
          setIndustryOrganizations(Array.isArray(items) ? items : []);
      })
      .catch(() => {
  ...
  const handleTriageSubmit = () => {
    ...
    setTimeout(() => {
      verifyIssue(issue.id, {
        ...
        nodalRemarks:
          nodalNote ||
          `Verified on ground by Nodal Desk. Assigned to ${assignedUniv}.`,
  ...
  const handleSponsor = async () => {
    ...
    try {
      const organizationName =
        currentUser?.organization || "Industry Workspace";
      const organization =
        industryOrganizations.find(
          (item) =>
      ...
      if (liveApi && !organization?.id)
        throw new Error(
          "No verified industry organization is available for this account.",
        );
      await sponsorProject(
        issue.assignedProject,
        sponsorAmount,
        organizationName,
        organization?.id,
      );
      ...
      >
        <div className="space-y-6">
            ...
            <div className="grid grid-cols-5 gap-1.5 sm:gap-2">
              {steps.map((step, idx) => {
                ...
                return (
                  ...
                  >
                    <div
                      className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold transition-all ${isPast ? "bg-jh-green-900 text-white" : isCurrent ? "bg-jh-terracotta-600 text-white ring-4 ring-jh-terracotta-100 animate-pulse" : "bg-jh-earth-200 text-jh-earth-600"}`}
                    >
                      <Icon className="w-4 h-4" />
                    </div>
                    <span
                      className={`text-[10px] font-semibold mt-1.5 line-clamp-1 ${isCurrent ? "text-jh-terracotta-700 font-bold" : isPast ? "text-jh-green-900" : "text-jh-earth-500"}`}
                    >
                      {step.label}
                    </span>
                  </div>
          ...
          <div className="p-4 rounded-2xl border border-jh-green-200 bg-jh-green-50/70 flex items-center justify-between gap-4">
            <div>
              <div className="flex items-center gap-2 text-xs font-bold text-jh-green-950">
                <BrainCircuit className="w-4 h-4 text-jh-green-700" />
                AI Issue Analysis
              </div>
              <p className="text-[11px] text-jh-green-900/70 mt-1">
                View the persisted classification, severity, summary, confidence
                and duplicate assessment for this case.
              </p>
            </div>
            <Button
              variant="secondary"
            ...
            <div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                {issue.images.map((img, i) => (
                  <div
                    key={i}
                    className="h-48 rounded-xl overflow-hidden border border-jh-earth-200 shadow-xs"
                  >
                    <img
                      src={img}
                      alt="Evidence"
            ...
            <div className="p-4 bg-white rounded-xl border border-jh-earth-200 space-y-2.5">
              ...
              <div className="text-xs space-y-1.5 text-jh-earth-800">
                ...
                <p className="text-jh-earth-600 pl-6">
                  District:{" "}
                  <strong className="text-jh-charcoal">{issue.district}</strong>
                </p>
                {issue.coordinates && (
                  <p className="text-jh-earth-500 text-[11px] pl-6 font-mono">
                    GPS: {issue.coordinates.lat.toFixed(4)}° N,{" "}
                    {issue.coordinates.lng.toFixed(4)}° E
                  </p>
                )}
              </div>
            </div>
            ...
            <div className="p-4 bg-blue-50/60 rounded-xl border border-blue-200 space-y-2">
              ...
              {issue.assignedUniversity && (
                <div className="pt-2 flex items-center gap-2 text-xs font-semibold text-jh-green-900">
                  ...
                  <span>
                    Assigned Academic Center: {issue.assignedUniversity}
                  </span>
                </div>
              )}
            </div>
            ...
            <div className="space-y-2">
              {issue.timeline &&
                issue.timeline.map((t, idx) => (
                  <div
                    key={idx}
                    className="flex items-start gap-3 text-xs bg-white p-2.5 rounded-lg border border-jh-earth-100"
                  >
                    <div className="w-2 h-2 rounded-full bg-jh-green-800 mt-1.5"></div>
                    <div className="flex-1">
                      <div className="flex items-center justify-between">
                        <span className="font-bold text-jh-green-950">
                          {t.status}
                        </span>
                        <span className="text-[10px] text-jh-earth-500">
                          {t.date}
                        </span>
                      </div>
          ...
          {currentRole === ROLES.NODAL && (
            <div className="p-4 bg-jh-green-50 rounded-2xl border border-jh-green-300 space-y-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div>
                  <label className="block text-[11px] font-bold text-jh-green-900 mb-1">
                    Update Status
                  </label>
                  <select
                    value={triageStatus}
              ...
              <div>
                <label className="block text-[11px] font-bold text-jh-green-900 mb-1">
                  Inspection Remarks & Instructions
                </label>
                <textarea
                  rows={2}
          ...
          {currentRole === ROLES.INDUSTRY && issue.assignedProject && (
            <div className="p-4 bg-orange-50 rounded-2xl border border-orange-200 space-y-3">
              ...
              <p className="text-xs text-orange-800">
                This issue has an active university R&D prototype ready for
                ground deployment. Funding required:{" "}
                <strong>{issue.fundingRequired || "₹ 5,00,000"}</strong>
              </p>
              {showSponsorSuccess ? (
                <div className="p-3 bg-emerald-100 text-emerald-800 text-xs font-bold rounded-lg text-center">
                  ✓ CSR Pledge of ₹{(Number(sponsorAmount) / 100000).toFixed(2)}{" "}
                  Lakh Confirmed! Transferred to University Escrow.
                </div>
              ) : (
                <div className="space-y-2">
                  {sponsorError && (
                    <div
                      role="alert"
                      className="p-2.5 rounded-lg border border-red-200 bg-red-50 text-red-800 text-xs"
                    >
                      {sponsorError}
                    </div>
                  )}

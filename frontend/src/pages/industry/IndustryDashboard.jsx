...
export const IndustryDashboard = ({ currentPath, onNavigate }) => {
  ...
  useEffect(() => {
    ...
    industryApi
      .listOrganizations()
      .then((items) => setOrganizations(Array.isArray(items) ? items : []))
  ...
  const handlePledgeSubmit = async (e) => {
    ...
    try {
      ...
      const org =
        organizations.find(
          (o) => String(o.name || "").toLowerCase() === orgName.toLowerCase(),
        ) || organizations[0];
      await sponsorProject(
        selectedProjectToFund.id,
        pledgeAmount,
        orgName,
        org?.id,
      );
    ...
    return (
      <div className="space-y-6">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            <h2 className="text-xl font-bold text-jh-green-950">
              Active CSR Sponsorships & Tranche Release
            </h2>
            <p className="text-xs text-jh-earth-600">
              Track company grants, Schedule VII tax receipts, and field
              milestones
            </p>
          </div>
          <Button
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
          {projects
            .filter((p) => p.sponsor && p.sponsor.includes("Tata"))
            .concat(projects.slice(0, 1))
            .map((prj) => (
              <div
                key={prj.id}
                className="bg-white rounded-2xl border border-jh-earth-200 p-6 shadow-jh-soft space-y-4"
              >
                <div className="flex items-start justify-between">
                  <div>
                    ...
                    <h3 className="text-base font-bold text-jh-green-950 mt-1">
                      {prj.title}
                    </h3>
                    <p className="text-xs text-jh-earth-600">
                      {prj.university} • Lead: {prj.studentLead}
                    </p>
                  </div>
                ...
                <div className="space-y-1.5 text-xs text-jh-earth-700 pt-2 border-t border-jh-earth-100">
                  <div className="flex justify-between">
                    ...
                    <strong className="font-mono text-jh-green-900">
                      ₹ {((Number(prj.budgetFunded) || 0) / 100000).toFixed(2)}{" "}
                      Lakh
                    </strong>
                  </div>
                  <div className="flex justify-between">
  ...
  if (currentPath === "csr-impact") {
    return (
      <div className="space-y-6">
        <div>
          <h2 className="text-xl font-bold text-jh-green-950">
            Corporate ESG Impact Scorecard
          </h2>
          <p className="text-xs text-jh-earth-600">
            Audited environmental indicators backed by university satellite and
            IoT telemetry
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-5">
          <div className="bg-white rounded-2xl border border-jh-earth-200 p-6 shadow-jh-soft text-center space-y-2">
            ...
            <h4 className="text-xs font-bold text-jh-green-950">
              Water Catchment Treated
            </h4>
            <p className="text-[11px] text-jh-earth-600">
              Constructed wetland biofiltration at Ghatshila & Jamshedpur
            </p>
          </div>
          ...
          <div className="bg-white rounded-2xl border border-jh-earth-200 p-6 shadow-jh-soft text-center space-y-2">
            ...
            <span className="text-2xl font-extrabold text-emerald-900 font-mono block">
              84 Hectares
            </span>
            <h4 className="text-xs font-bold text-jh-green-950">
              Mine Overburden Reclaimed
            </h4>
            <p className="text-[11px] text-jh-earth-600">
              Deep-root vetiver phytoremediation and biochar soil stabilization
            </p>
          </div>
          ...
          <div className="bg-white rounded-2xl border border-jh-earth-200 p-6 shadow-jh-soft text-center space-y-2">
            ...
            <span className="text-2xl font-extrabold text-amber-900 font-mono block">
              94.8%
            </span>
            <h4 className="text-xs font-bold text-jh-green-950">
            ...
            <p className="text-[11px] text-jh-earth-600">
              State Principal Secretary audited CSR tranche deployment
            </p>
          </div>
        </div>
      </div>
    ...
    <div className="space-y-8">
      <div className="bg-white rounded-3xl p-6 sm:p-8 border border-jh-earth-200 shadow-jh-soft flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        <div className="space-y-1.5">
          <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-orange-100 text-orange-900 text-xs font-semibold">
            ...
            <span>
              Corporate CSR & ESG Command Desk •{" "}
              {currentUser.organization || "Tata Steel Foundation"}
            </span>
          </div>
          ...
          <p className="text-xs text-jh-earth-600 max-w-xl">
            {currentUser.title || "Chief CSR & Sustainability Officer"} — Direct
            corporate capital toward vetted university R&D solutions that
            deliver high-impact ESG compliance across Jharkhand.
          </p>
        </div>
        <div className="bg-jh-earth-50 p-3.5 rounded-2xl border border-jh-earth-200 text-right">
          <span className="text-[10px] uppercase font-bold text-jh-earth-600 block">
            Total CSR Budget
          </span>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard
          ...
          value={`₹ ${(sponsors.reduce((sum, s) => sum + Number(s.amount || 0), 0) / 10000000).toFixed(2)} Cr`}
          subtitle="Live sponsorship records"
          icon={Coins}
          color="terracotta"
        />
        <StatCard
      ...
      <div className="space-y-4">
        <div className="flex flex-wrap items-center justify-between gap-3 pb-2 border-b border-jh-earth-200">
          <div>
            ...
            <p className="text-xs text-jh-earth-600">
              Browse scientifically validated prototypes awaiting corporate
              funding tranches
            </p>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
            {filteredProjects.map((project) => (
              <ProjectCard
                key={project.id}
                project={project}
                onSponsorClick={setSelectedProjectToFund}
              />
            ))}
          </div>
        )}
        ...
        >
          {pledgeSuccess ? (
            <div className="text-center py-6 space-y-3">
              ...
              <h3 className="text-lg font-bold text-jh-green-950">
                CSR Grant Submitted
              </h3>
              <p className="text-xs text-jh-earth-700">
                The sponsorship has been recorded and mapped to the selected
                project.
              </p>
            </div>
          ) : (
            <form onSubmit={handlePledgeSubmit} className="space-y-4 text-xs">
              {error && (
                <div
                  role="alert"
                  className="p-3 rounded-xl border border-red-200 bg-red-50 text-red-800"
                >
                  {error}
                </div>
              )}
              <div className="p-3 bg-jh-earth-50 rounded-xl border border-jh-earth-200">
                <span className="text-[10px] uppercase font-bold text-jh-earth-500">
                  Academic Host:
                </span>
                <p className="font-bold text-jh-green-950">
                  {selectedProjectToFund.university || "Not specified"}
                </p>
                <p className="text-jh-earth-600">
                  Total Budget: ₹{" "}
                  {(
                    (Number(selectedProjectToFund.budgetTotal) || 0) / 100000
                  ).toFixed(2)}{" "}
                  Lakh
                </p>
              </div>
              <div>
                <label className="block font-bold text-jh-charcoal mb-1">
                  Pledge Grant Amount (in INR) *
                </label>
                <input
                  type="number"
                  required
              ...
              <div className="p-3 bg-orange-50 border border-orange-200 rounded-xl text-orange-950">
                ...
                <p className="text-[11px] mt-1">
                  Contribution records remain subject to backend approval and
                  applicable CSR compliance.
                </p>
              </div>
              <div className="flex justify-end gap-2 pt-2">
                <Button
                  ...
                  size="sm"
                  type="button"
                  onClick={() => setSelectedProjectToFund(null)}
                >
                  Cancel
                </Button>
                <Button

...
export const AdminDashboard = ({ currentPath }) => {
  ...
  useEffect(() => {
    if (!liveApi) return;
    Promise.allSettled([dashboardApi.getIssueCategories(), dashboardApi.getIssueStatus()]).then(([categories, statuses]) => {
      setAnalytics({
  ...
  const loadUsers = async () => {
    ...
    try { const data = await usersApi.listUsers(); setUsers(Array.isArray(data) ? data : []); }
    catch (error) { setUserError(error.message || 'Unable to load users.'); }
    finally { setUsersLoading(false); }
  };

  useEffect(() => { if (activeAdminTab === 'users') loadUsers(); }, [activeAdminTab, liveApi]);

  const domainChartData = useMemo(() => analytics.categories && typeof analytics.categories === 'object'
    ? Object.entries(analytics.categories).map(([name, value]) => ({ name: formatLabel(name), issues: Number(value) || 0 })).sort((a,b) => b.issues-a.issues).slice(0,8)
    : [], [analytics.categories]);
  const statusPieData = useMemo(() => analytics.statuses && typeof analytics.statuses === 'object'
    ? Object.entries(analytics.statuses).map(([name, value]) => ({ name: formatLabel(name), value: Number(value) || 0 })).filter(x => x.value > 0).sort((a,b) => b.value-a.value)
    : [], [analytics.statuses]);
  const filteredUsers = useMemo(() => {
    const q = userSearch.trim().toLowerCase();
    if (!q) return users;
    return users.filter(u => [u.name, u.email, u.role].some(v => String(v || '').toLowerCase().includes(q)));
  }, [users, userSearch]);

  const createUser = async event => {
    event.preventDefault(); setSavingUser(true); setUserError('');
    try {
      const created = await usersApi.createUser(form);
      setUsers(prev => [created, ...prev]);
      setForm({ name: '', email: '', password: '', role: 'NODAL_OFFICER' });
      setShowAddUser(false);
    } catch (error) { setUserError(error.message || 'Unable to create user.'); }
  ...
  return <div className="space-y-8">
    <div className="bg-white rounded-3xl p-6 sm:p-8 border border-jh-earth-200 shadow-jh-soft flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
      <div><div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-red-100 text-red-900 text-xs font-semibold"><ShieldCheck className="w-3.5 h-3.5" />State Command Center</div><h1 className="text-2xl sm:text-3xl font-bold text-jh-green-950 mt-2">{currentUser?.name || 'Administrator'}</h1><p className="text-xs text-jh-earth-600 mt-1">Manage platform operations, users, roles and statewide civic intelligence.</p></div>
      <span className={`px-3.5 py-1.5 rounded-xl text-xs font-bold ${liveApi ? 'bg-emerald-50 border border-emerald-300 text-emerald-800' : 'bg-amber-50 border border-amber-300 text-amber-800'}`}>{liveApi ? 'Live Platform Data' : 'Live API Required'}</span>
    </div>

    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
      <StatCard title="Total State Issues" value={totalIssueCount} subtitle={dashboard?.openIssues != null ? `${dashboard.openIssues} currently open` : 'Live issue records'} icon={Activity} color="forest" />
      <StatCard title="CSR Capital Deployed" value={stats?.totalCSRFundingSanctioned ?? '—'} subtitle={organizationCount ? `${organizationCount} organizations registered` : 'Live sponsorship records'} icon={Building} color="terracotta" />
      <StatCard title="Active University Labs" value={activeProjectCount} subtitle={dashboard?.projects != null ? `${dashboard.projects} total projects` : 'Live project count'} icon={School} color="blue" />
      <StatCard title="Ecology Restored" value={stats?.forestWaterAreaRestoredSqKm ?? '—'} subtitle="Live platform metric" icon={Globe2} color="gold" />
    </div>

    <div className="flex items-center gap-2 border-b border-jh-earth-200 pb-2 overflow-x-auto">
      <button onClick={() => setActiveAdminTab('overview')} className={`px-4 py-2 rounded-xl text-xs font-bold whitespace-nowrap ${activeAdminTab === 'overview' ? 'bg-jh-green-900 text-white' : 'text-jh-earth-700 hover:bg-jh-earth-100'}`}>Executive Analytics & Charts</button>
      <button onClick={() => setActiveAdminTab('heatmaps')} className={`px-4 py-2 rounded-xl text-xs font-bold whitespace-nowrap ${activeAdminTab === 'heatmaps' ? 'bg-jh-green-900 text-white' : 'text-jh-earth-700 hover:bg-jh-earth-100'}`}>State GIS Heatmap</button>
      <button onClick={() => setActiveAdminTab('users')} className={`px-4 py-2 rounded-xl text-xs font-bold whitespace-nowrap ${activeAdminTab === 'users' ? 'bg-jh-green-900 text-white' : 'text-jh-earth-700 hover:bg-jh-earth-100'}`}>User & Role Governance</button>
    </div>

    {activeAdminTab === 'overview' && <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
      <div className="lg:col-span-7 bg-white rounded-2xl border border-jh-earth-200 p-5 shadow-jh-soft"><h3 className="text-xs font-bold uppercase tracking-wider text-jh-green-950 mb-4">Interventions by Ecological Domain</h3>{domainChartData.length ? <div className="h-64"><ResponsiveContainer width="100%" height="100%"><BarChart data={domainChartData}><XAxis dataKey="name" tick={{fontSize:11}} /><YAxis allowDecimals={false} tick={{fontSize:11}} /><Tooltip /><Bar dataKey="issues" radius={[6,6,0,0]} /></BarChart></ResponsiveContainer></div> : <div className="h-64 grid place-items-center text-xs text-jh-earth-500">No live analytics available.</div>}</div>
      <div className="lg:col-span-5 bg-white rounded-2xl border border-jh-earth-200 p-5 shadow-jh-soft"><h3 className="text-xs font-bold uppercase tracking-wider text-jh-green-950 mb-4">State Grievance Lifecycle Status</h3>{statusPieData.length ? <><div className="h-56"><ResponsiveContainer width="100%" height="100%"><PieChart><Pie data={statusPieData} cx="50%" cy="50%" innerRadius={55} outerRadius={80} paddingAngle={4} dataKey="value">{statusPieData.map((_, i) => <Cell key={i} />)}</Pie><Tooltip /></PieChart></ResponsiveContainer></div><div className="grid grid-cols-2 gap-2 text-[10.5px]">{statusPieData.map(d => <div key={d.name} className="font-medium text-jh-earth-800 truncate">{d.name} ({d.value})</div>)}</div></> : <div className="h-64 grid place-items-center text-xs text-jh-earth-500">No live analytics available.</div>}</div>
    </div>}

    {activeAdminTab === 'heatmaps' && <IssueMap issues={issues} height="600px" />}

    {activeAdminTab === 'users' && <div className="bg-white rounded-2xl border border-jh-earth-200 p-5 shadow-jh-soft">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-4"><div><h3 className="text-xs font-bold uppercase tracking-wider text-jh-green-950">User & Role Governance</h3><p className="text-[11px] text-jh-earth-600 mt-1">Create every operational role except Citizen. Credentials are stored by the backend.</p></div><button onClick={() => setShowAddUser(true)} disabled={!liveApi} className="inline-flex items-center justify-center gap-2 px-3 py-2 rounded-xl bg-jh-green-900 text-white text-xs font-bold disabled:opacity-50"><Plus className="w-4 h-4" />Add User</button></div>

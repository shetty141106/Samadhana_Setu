      ...
      <div className="bg-white rounded-3xl p-6 border border-jh-earth-200 shadow-jh-soft flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-jh-earth-600 mt-1">
            Live issue records pulled from the SamadhanSetu database.
          </p>
        </div>
        <div className="text-right">
          <div className="text-2xl font-bold text-jh-green-950">
            {issues.length}
          </div>
          <div className="text-[10px] uppercase tracking-wider text-jh-earth-500">
            Total Issues
          </div>
        </div>
      </div>
      ...
      <div className="bg-white rounded-2xl border border-jh-earth-200 shadow-jh-soft p-4">
        <div className="flex flex-col sm:flex-row gap-3">
          <div className="relative flex-1">
            ...
            <input
              ...
              placeholder="Search by issue, category, district, status or priority..."
              className="w-full pl-9 pr-4 py-2.5 text-xs bg-jh-earth-50 border border-jh-earth-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-jh-green-700/40"
            />
          </div>
          <button
            onClick={refreshIssues}
            disabled={!liveApi || dataLoading}
            className="inline-flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl bg-jh-green-900 text-white text-xs font-bold disabled:opacity-50"
          >
            <RefreshCw
        ...
        {!liveApi && (
          <div className="mt-3 rounded-xl border border-amber-200 bg-amber-50 px-3 py-2 text-xs text-amber-800">
            Live API is disabled. Enable live API integration to load database
            issues.
          </div>
        )}
      </div>
            ...
            <tbody className="divide-y divide-jh-earth-100">
              {dataLoading ? (
                <tr>
                  <td
                    colSpan="8"
                    className="p-10 text-center text-jh-earth-500"
                  >
                ...
                filteredIssues.map((issue) => (
                  <tr key={issue.id} className="hover:bg-jh-earth-50">
                    <td className="p-3 font-mono text-jh-earth-600">
                      #{issue.id}
                    </td>
                    <td className="p-3 min-w-[220px]">
                      <div className="font-bold text-jh-green-950">
                        {issue.title || "Untitled issue"}
                      </div>
                      <div className="mt-1 text-[10px] text-jh-earth-500 line-clamp-2">
                        {issue.description || "No description"}
                      </div>
                    </td>
                    <td className="p-3">
                      {label(issue.categoryLabel || issue.category)}
                    </td>
                    <td className="p-3">{issue.district || "—"}</td>
                    <td className="p-3">
                      <span className="px-2 py-1 rounded-full bg-jh-earth-100 font-semibold">
                        {label(issue.status)}
                      </span>
                    </td>
                    <td className="p-3">
                    ...
                    <td className="p-3">
                      {issue.submittedBy || issue.citizenName || "—"}
                    </td>
                    <td className="p-3">
                      <button
                        onClick={() => setSelectedIssue(issue)}
                        className="inline-flex items-center gap-1 px-2.5 py-1.5 rounded-lg bg-jh-green-50 text-jh-green-900 font-bold hover:bg-jh-green-100"
                      >

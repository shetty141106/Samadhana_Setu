...
export const Login = ({ onNavigate }) => {
  ...
  const [email, setEmail] = useState('birsa.munda@jharkhand.in');
  ...
  const getRoleIcon = (roleId) => {
    switch (roleId) {
      ...
      case ROLES.INDUSTRY: return Building2;
  ...
  return (
    <div className="min-h-[85vh] flex items-center justify-center py-12 px-4 sm:px-6 lg:px-8 relative overflow-hidden bg-jh-earth-100">
      <div className="absolute inset-0 z-0">
        ...
        <div className="absolute inset-0 bg-forest-overlay opacity-90"></div>
      </div>
      ...
      <div className="relative z-10 max-w-4xl w-full grid grid-cols-1 md:grid-cols-12 gap-6 bg-white/95 backdrop-blur-md rounded-3xl shadow-2xl border border-jh-earth-200 overflow-hidden">
        <div className="md:col-span-7 p-6 sm:p-8 bg-jh-earth-50/80 border-b md:border-b-0 md:border-r border-jh-earth-200 flex flex-col justify-between">
          <div>
            <div className="flex items-center gap-2 text-xs font-bold uppercase tracking-wider text-jh-terracotta-700 mb-1"><Sparkles className="w-4 h-4" /><span>Interactive Persona Demo Launcher</span></div>
            <h3 className="text-xl font-bold text-jh-green-950 mb-1">Select a Role to Preview</h3>
            <p className="text-xs text-jh-earth-600 mb-6">Instant demo access remains available; standard sign-in uses the Spring Boot JWT API when live API mode is enabled.</p>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              {Object.values(ROLES).map((roleKey) => {
                const cfg = ROLE_CONFIGS[roleKey];
                const Icon = getRoleIcon(roleKey);
                return <button key={roleKey} type="button" onClick={() => handleQuickLogin(roleKey)} className="text-left p-3 rounded-2xl border border-jh-earth-200 bg-white hover:border-jh-green-700 hover:shadow-jh-soft transition-all duration-200 flex items-start gap-3 group"><div className="w-9 h-9 rounded-xl bg-jh-green-50 text-jh-green-900 flex items-center justify-center flex-shrink-0 group-hover:bg-jh-green-900 group-hover:text-white transition-colors"><Icon className="w-5 h-5" /></div><div className="min-w-0"><h4 className="text-xs font-bold text-jh-green-950 group-hover:text-jh-terracotta-700 truncate">{cfg.label}</h4><p className="text-[10px] text-jh-earth-600 truncate">{cfg.portalTitle}</p></div></button>;
              })}
            </div>
          </div>
          <div className="pt-6 mt-4 border-t border-jh-earth-200 flex items-center justify-between text-xs text-jh-earth-600"><span>Govt of Jharkhand Unified SSO</span><span className="font-semibold text-jh-green-900">JWT / Security Standard v2.4</span></div>
        </div>

        <div className="md:col-span-5 p-6 sm:p-8 flex flex-col justify-between">
          <div>
            <div className="mb-6"><SamadhanLogo size="md" /><p className="text-xs text-jh-earth-600 mt-2">Official single sign-on portal for citizens, officers, researchers, and CSR sponsors.</p></div>
            ...
            {isAuthenticated && <div className="mb-4 rounded-xl border border-emerald-200 bg-emerald-50 px-3 py-2 text-xs text-emerald-800">You are already signed in. You can continue to a role workspace.</div>}
            <form onSubmit={handleStandardSubmit} className="space-y-4">
              <div><label className="block text-[11px] font-bold uppercase tracking-wider text-jh-earth-800 mb-1">Email / Official ID</label><div className="relative"><Mail className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-jh-earth-500" /><input type="email" required value={email} onChange={(e) => setEmail(e.target.value)} className="w-full pl-9 pr-3 py-2 text-xs bg-jh-earth-50 border border-jh-earth-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-jh-green-700" /></div></div>

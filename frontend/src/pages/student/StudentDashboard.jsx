...
export const StudentDashboard = ({ currentPath, onNavigate }) => {
  ...
  const activeProject =
    projects.find((p) => p.id === currentUser.project) || projects[0] || null;

  if (!activeProject) {
    return (
      <div className="space-y-6">
        <div className="bg-white rounded-3xl p-8 border border-jh-earth-200 shadow-jh-soft">
          ...
          <h1 className="mt-4 text-2xl font-bold text-jh-green-950">
            No research project assigned yet
          </h1>
          <p className="mt-2 text-sm text-jh-earth-600 max-w-xl">
            Your student workspace is ready. A faculty/admin assignment is
            required before project and Kanban data can be displayed.
          </p>
        </div>
      </div>
    );
  ...
  const doneTasks =
    activeProject.kanbanTasks?.filter(
    ...
    return (
      <div className="max-w-2xl mx-auto py-12 text-center bg-white rounded-3xl border border-jh-earth-200 p-8 shadow-jh-soft space-y-4">
        ...
        <h2 className="text-xl font-bold text-jh-green-950">
          Awaiting University Lab Project Assignment
        </h2>
        <p className="text-xs text-jh-earth-600 max-w-md mx-auto leading-relaxed">
          Your profile is currently active in the{" "}
          {currentUser.university || "IIT (ISM) Dhanbad"} student researcher
          pool. Once your department faculty mentor assigns an active grievance
          solution, your sprint Kanban will unlock here.
        </p>
  ...
  if (currentPath === "team-roster") {
    return (
      <div className="space-y-6">
        <div>
          <h2 className="text-xl font-bold text-jh-green-950">
            Research Lab Team Roster
          </h2>
          <p className="text-xs text-jh-earth-600">
            Active scholars collaborating on project {activeProject.id}
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
          {activeProject.teamMembers?.map((member, idx) => (
            <div
              key={idx}
              className="bg-white rounded-2xl border border-jh-earth-200 p-5 shadow-jh-soft flex items-center gap-3.5"
            >
              ...
              <div className="min-w-0">
                <h4 className="text-sm font-bold text-jh-green-950 truncate">
                  {member.name}
                </h4>
                <p className="text-xs text-jh-earth-600 truncate">
                  {member.role}
                </p>
    ...
    <div className="space-y-8">
      <div className="bg-white rounded-3xl p-6 sm:p-8 border border-jh-earth-200 shadow-jh-soft flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        <div className="space-y-1.5">
          <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-amber-100 text-amber-900 text-xs font-semibold">
            <Sparkles className="w-3.5 h-3.5" />
            Student Innovation Lab •{" "}
            {currentUser.university || "IIT (ISM) Dhanbad"}
          </div>
          <h1 className="text-2xl sm:text-3xl font-bold text-jh-green-950">
            {currentUser.name}
          </h1>
          <p className="text-xs text-jh-earth-600 max-w-xl">
            Lead Student Researcher for project{" "}
            <strong className="text-jh-green-900">{activeProject.id}</strong> —
            Developing sustainable bio-technological solutions for Jharkhand's
            river catchments.
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <StatCard
          ...
          value={`${activeProject.teamMembers?.length || 0} Scholars`}
        ...
        <StatCard
          ...
          value={activeProject.facultyMentor || "Assigned Faculty"}
          ...
          <div className="bg-white rounded-2xl border border-jh-earth-200 p-5 shadow-jh-soft space-y-4">
            ...
            <div className="space-y-2 text-xs text-jh-earth-700 pt-2 border-t border-jh-earth-100">
              <div className="flex items-center justify-between gap-3">
                ...
                <strong className="text-jh-charcoal text-right">
                  {activeProject.university || "Not assigned"}
                </strong>
              </div>
              <div className="flex items-center justify-between gap-3">
                <span>CSR Industry Partner:</span>
                <strong className="text-jh-terracotta-700 text-right">
                  {activeProject.sponsor || "Not funded"}
                </strong>
              </div>
              <div className="flex items-center justify-between gap-3">
                <span>Sanctioned Grant:</span>
                <strong className="text-jh-green-900 font-mono">
                  ₹{" "}
                  {((Number(activeProject.budgetFunded) || 0) / 100000).toFixed(
                    1,
                  )}{" "}
                  Lakh
                </strong>
            ...
            <div className="pt-3 border-t border-jh-earth-100">
              <h4 className="text-xs font-bold uppercase tracking-wider text-jh-earth-800 mb-2">
                Team Roster ({activeProject.teamMembers?.length || 0})
              </h4>
              <div className="space-y-2">
                {activeProject.teamMembers?.map((m, idx) => (
                  <div
                    key={m.id || idx}
                    className="flex items-center justify-between text-xs bg-jh-earth-50 p-2 rounded-lg"
                  >
                    <span className="font-bold text-jh-charcoal">{m.name}</span>
                    <span className="text-[10.5px] text-jh-earth-600 truncate max-w-[140px]">
                      {m.role}
                    </span>
                  </div>
                ))}

...
public class IndustryService {
    ...
    public SponsorshipResponseDto sponsor(SponsorshipRequestDto r) {
        ...
        Sponsorship saved = sponsorships.save(Sponsorship.builder().organization(o).project(p).amount(r.getAmount()).status(r.getStatus() == null ? "PENDING" : r.getStatus()).build());
        return smap(saved);
    }
    ...
    public List<SponsorshipResponseDto> sponsorships() {
        ...
        Optional<Industry> industry = industries.findByUserEmail(currentEmail());
        if (industry.isEmpty() || industry.get().getOrganization() == null) return List.of();
        Long organizationId = industry.get().getOrganization().getId();
    ...
    private Organization findOwnedOrg(Long id) { Organization organization = findOrg(id); if (!isAdmin() && !ownsOrganization(id)) throw new IllegalArgumentException("You cannot modify another organization's record"); return organization; }
    private boolean ownsOrganization(Long organizationId) { Industry industry = currentIndustry(); return industry.getOrganization() != null && organizationId != null && organizationId.equals(industry.getOrganization().getId()); }
    private Industry currentIndustry() { return industries.findByUserEmail(currentEmail()).orElseThrow(() -> new IllegalArgumentException("Industry profile is not configured for this account")); }
    private String currentEmail() { Authentication authentication = SecurityContextHolder.getContext().getAuthentication(); if (authentication == null || authentication.getName() == null || authentication.getName().isBlank()) throw new IllegalArgumentException("Authenticated user is required"); return authentication.getName(); }
    ...
    private void assertSponsorshipAccess(Sponsorship sponsorship) { if (!isAdmin() && (sponsorship.getOrganization() == null || !ownsOrganization(sponsorship.getOrganization().getId()))) throw new IllegalArgumentException("You cannot access another organization's sponsorship"); }

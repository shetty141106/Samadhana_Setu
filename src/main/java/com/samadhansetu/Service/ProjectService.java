...
public class ProjectService {
    ...
    public ProjectResponseDto create(ProjectRequestDto r, Authentication authentication) {
        ...
        if (r.getSourceIssueId() != null) {
            ...
            if (sourceIssue.getStatus() != IssueStatus.VERIFIED)
                throw new IllegalArgumentException("Only Nodal-verified issues can create projects");
            p.setSourceIssueId(sourceIssue.getId());
        }
        if (r.getUniversityId() != null)
            p.setUniversity(universities.findById(r.getUniversityId())
    ...
    public List<ProjectResponseDto> byStatus(ProjectStatus s) {
        return projects.findByStatus(s).stream().map(this::map).toList();
    }

    public ProjectResponseDto update(Long id, ProjectRequestDto r, Authentication authentication) {
        Project p = find(id);
        assertProjectMutationAccess(p, authentication);
    ...
    public TeamMemberResponseDto addMember(Long projectId, TeamMemberRequestDto r, Authentication authentication) {
        ...
        if (members.existsByProjectIdAndUserId(projectId, r.getUserId()))
            throw new IllegalArgumentException("User already belongs to project");
        return memberMap(members.save(TeamMember.builder().project(p).user(u).memberRole(r.getMemberRole()).build()));
    }

    public List<TeamMemberResponseDto> members(Long id) {
        return members.findByProjectId(id).stream().map(this::memberMap).toList();
    }

    public void removeMember(Long id, Authentication authentication) {
        TeamMember member = members.findById(id)
                .orElseThrow(() -> new IllegalArgumentException("Team member not found: " + id));
        assertProjectMutationAccess(member.getProject(), authentication);
    ...
    public MilestoneResponseDto addMilestone(Long projectId, MilestoneRequestDto r, Authentication authentication) {
        Project p = find(projectId);
        assertProjectMutationAccess(p, authentication);
        Milestone m = Milestone.builder().project(p).title(r.getTitle()).startDate(r.getStartDate())
                .endDate(r.getEndDate())
                .status(r.getStatus() == null ? MilestoneStatus.PENDING : r.getStatus()).build();
        return milestoneMap(milestones.save(m));
    }

    public List<MilestoneResponseDto> milestones(Long id) {
        return milestones.findByProjectId(id).stream().map(this::milestoneMap).toList();
    }

    public MilestoneResponseDto updateMilestone(Long id, MilestoneRequestDto r, Authentication authentication) {
        Milestone m = milestones.findById(id)
    ...
    public TaskResponseDto addTask(Long projectId, TaskRequestDto r, Authentication authentication) {
        ...
        if (r.getMilestoneId() != null)
            t.setMilestone(findMilestoneForProject(r.getMilestoneId(), projectId));
        if (r.getAssignedToId() != null)
            t.setAssignedTo(findProjectMember(projectId, r.getAssignedToId()));
        return taskMap(tasks.save(t));
    }

    public List<TaskResponseDto> tasks(Long id) {
        return tasks.findByProjectId(id).stream().map(this::taskMap).toList();
    }
    ...
    public TaskResponseDto updateTask(Long id, TaskRequestDto r, Authentication authentication) {
        ...
        // Students may advance/reverse the status of tasks assigned to them, but may
        // not
        // edit project structure, task ownership, title, dates, or milestone
        // assignment.
        ...
        if (r.getStatus() != null)
            t.setStatus(r.getStatus());
        t.setMilestone(
                r.getMilestoneId() == null ? null : findMilestoneForProject(r.getMilestoneId(), project.getId()));
    ...
    public void deleteTask(Long id, Authentication authentication) {
        Task t = tasks.findById(id).orElseThrow(() -> new IllegalArgumentException("Task not found: " + id));
        assertProjectMutationAccess(t.getProject(), authentication);
        tasks.delete(t);
    }

    private void assertStaff(Authentication authentication) {
        if (authentication == null || !(hasRole(authentication, "ADMIN") || hasRole(authentication, "NODAL_OFFICER")
                || hasRole(authentication, "FACULTY")))
    ...
    private Project find(Long id) {
        return projects.findById(id).orElseThrow(() -> new IllegalArgumentException("Project not found: " + id));
    }
    ...
    private User findProjectMember(Long projectId, Long userId) {
        ...
        if (!members.existsByProjectIdAndUserId(projectId, userId))
            throw new IllegalArgumentException("Assigned user is not a member of project: " + projectId);
        return user;
    }

    private ProjectResponseDto map(Project p) {
        return ProjectResponseDto.builder().id(p.getId()).title(p.getTitle()).description(p.getDescription())
    ...
    private TeamMemberResponseDto memberMap(TeamMember m) {
        return TeamMemberResponseDto.builder().id(m.getId()).projectId(m.getProject().getId())
                .userId(m.getUser().getId()).userName(m.getUser().getName()).memberRole(m.getMemberRole()).build();
    }

    private MilestoneResponseDto milestoneMap(Milestone m) {
        return MilestoneResponseDto.builder().id(m.getId()).projectId(m.getProject().getId()).title(m.getTitle())
                .startDate(m.getStartDate()).endDate(m.getEndDate()).status(m.getStatus())
                .taskCount(tasks.findByMilestoneId(m.getId()).size()).build();
    }

    private TaskResponseDto taskMap(Task t) {
        return TaskResponseDto.builder().id(t.getId()).projectId(t.getProject().getId())

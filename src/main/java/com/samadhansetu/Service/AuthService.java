...
public class AuthService {
    ...
    public AuthResponse register(RegisterRequest request) {
        ...
        Role role = roleRepository.findByName("CITIZEN").orElseGet(
                () -> roleRepository.save(Role.builder().name("CITIZEN").description("Citizen user").build()));
        User saved = userRepository.save(User.builder().name(request.getName()).email(request.getEmail())
    ...
    @Transactional
    public AuthResponse updateProfile(ProfileUpdateRequest request, String currentEmail) {
    ...
    private AuthResponse toResponse(User user, String token) {
        return AuthResponse.builder().token(token).userId(user.getId()).name(user.getName()).email(user.getEmail())
                .role(user.getRole() == null ? null : user.getRole().getName()).build();
    }
}

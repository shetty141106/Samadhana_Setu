...
public class IssueService {
    ...
    public IssueResponseDto create(IssueRequestDto request, String email) {
        ...
        Citizen citizen = citizenRepository.findByUserEmail(email)
                .orElseGet(() -> citizenRepository.save(Citizen.builder().user(user).build()));
        ...
        if (request.getEvidenceMedia() != null)
            request.getEvidenceMedia().forEach(e -> issue.getEvidenceMedia().add(EvidenceMedia.builder().issue(issue)
                    .mediaUrl(e.getMediaUrl()).mediaType(e.getMediaType()).build()));
    ...
    public IssueResponseDto getById(Long id, String email, boolean staff) {
        ...
        if (!staff && !isOwnedBy(issue, email))
            throw new AccessDeniedException("You can only access your own issues");
        return toDto(issue);
    }
    ...
    public List<IssueResponseDto> getByPriority(IssuePriority p) {
        return issueRepository.findByPriority(p).stream().map(this::toDto).toList();
    }

    @Transactional
    public IssueResponseDto update(Long id, IssueRequestDto r) {
        Issue i = find(id);
        i.setTitle(r.getTitle());
        i.setDescription(r.getDescription());
        i.setLocation(r.getLocation());
    ...
    @Transactional
    public IssueResponseDto updatePriority(Long id, IssuePriority p) {
        Issue i = find(id);
        i.setPriority(p);
        return toDto(issueRepository.save(i));
    }
    ...
    @Transactional
    public IssueResponseDto addEvidence(Long id, EvidenceMediaDto d) {
        Issue i = find(id);
        i.getEvidenceMedia()
                .add(EvidenceMedia.builder().issue(i).mediaUrl(d.getMediaUrl()).mediaType(d.getMediaType()).build());
        return toDto(issueRepository.save(i));
    }

    public void delete(Long id) {
        issueRepository.delete(find(id));
    }

    private Issue find(Long id) {
        return issueRepository.findById(id).orElseThrow(() -> new IllegalArgumentException("Issue not found: " + id));
    }

    private boolean isOwnedBy(Issue issue, String email) {
        return issue.getReportedBy() != null && issue.getReportedBy().getUser() != null
                && email.equalsIgnoreCase(issue.getReportedBy().getUser().getEmail());
    }

    private IssueResponseDto toDto(Issue i) {
        return IssueResponseDto.builder().id(i.getId()).title(i.getTitle()).description(i.getDescription())
                ...
                .evidenceMedia(i.getEvidenceMedia() == null ? List.of()
                        : i.getEvidenceMedia().stream()
                                .map(e -> EvidenceMediaDto.builder().mediaUrl(e.getMediaUrl())
                                        .mediaType(e.getMediaType()).build())
                                .toList())
                .aiAnalysis(aiBridgeService.getLatestAnalysis(i.getId())).build();
    }
}

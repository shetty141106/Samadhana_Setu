...
public class DashboardService {
 ...
 public Map<String,Object> summary(){
  ...
  m.put("issues",issues.count());
  m.put("projects",projects.count());
  m.put("universities",universities.count());
  m.put("organizations",organizations.count());
  m.put("sponsorships",sponsorships.count());
  m.put("tasks",tasks.count());
  m.put("openIssues",issues.findAll().stream().filter(i->i.getStatus()!=IssueStatus.RESOLVED&&i.getStatus()!=IssueStatus.REJECTED).count());
  m.put("activeProjects",projects.findByStatus(ProjectStatus.ACTIVE).size());

  // Public landing-page metrics. These are read directly from the database.
  m.put("totalIssuesVerified",issues.findAll().stream().filter(i->i.getStatus()==IssueStatus.VERIFIED).count());
  m.put("activeUniversityProjects",projects.findByStatus(ProjectStatus.ACTIVE).stream().filter(p->p.getUniversity()!=null).count());
  m.put("totalCSRFundingSanctioned",sponsorships.findAll().stream().map(s->s.getAmount()==null?java.math.BigDecimal.ZERO:s.getAmount()).reduce(java.math.BigDecimal.ZERO,java.math.BigDecimal::add));
  // No persisted ecology-restoration metric exists yet, so the frontend receives 0 instead of a fabricated value.
  m.put("forestWaterAreaRestoredSqKm",0);
  return m;
 }
